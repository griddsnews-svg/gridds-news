...
const ALLOWED_EVENTS = ['open', 'fullstory_click', 'ad_impression', 'ad_click'];
...
export default async function handler(req, res) {
  ...
  // Ad events bump atomic counters on the ads table via an RPC (no story_events row).
  if (eventType === 'ad_impression' || eventType === 'ad_click') {
    const adId = body.ad_id ? String(body.ad_id) : null;
    if (!adId || !uuidRe.test(adId)) {
      return res.status(400).json({ error: 'Invalid ad_id' });
    }
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/gridds_bump_ad`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          p_ad_id:  adId,
          p_metric: eventType === 'ad_click' ? 'click' : 'impression',
        }),
      });
      if (!r.ok) {
        const t = await r.text();
        return res.status(502).json({ error: 'Ad bump failed', detail: t.slice(0, 200) });
      }
      return res.status(204).end();
    } catch (e) {
      return res.status(500).json({ error: 'Ad track failed', detail: e.message });
    }
  }
