...
//
// RELIABILITY MODEL (v3) — why cards used to fail for most publishers:
//   satori + resvg can ONLY rasterise JPEG/PNG/GIF. They CANNOT decode WebP or
//   AVIF. Most Indian publishers (and most manually-uploaded images) are WebP,
//   so satori threw, the handler 500'd, and the client silently fell back to a
//   text-only share. Only JPEG sources (The Hindu, Mint, Indian Express) worked.
//
//   Fix: every image is decoded + re-encoded to a clean JPEG via sharp BEFORE it
//   reaches satori (handles WebP/AVIF/CMYK/progressive/oversized/animated). And
//   the handler NEVER errors for a renderable story — if the photo can't be
//   loaded or processed, it renders on the gradient fallback instead of 500-ing.
//   Net effect: a valid PNG is ALWAYS returned, so the client always shares a card.
//
...
// Hero box the photo is cropped to (top half of the 1080×1440 card).
const IMG_W = 1080, IMG_H = 620;
...
export async function renderCard({ headline, summary, source, dateLabel, sectionId, imageDataUri, theme }) {
  ...
  const imageBlock = imageDataUri
    ? h('img', { src: imageDataUri, width: IMG_W, height: IMG_H, style: { objectFit: 'cover' } })
...
// ── Load the article image and return a satori-safe data URI, or null. ──
// This is the core of the fix. We fetch the raw bytes (Supabase upload OR
// publisher URL) and ALWAYS re-encode through sharp to a clean JPEG, cropped to
// the hero box. sharp decodes WebP/AVIF/PNG/JPEG/CMYK/progressive; satori only
// ever sees a plain JPEG, so it can never throw on an "unsupported image type".
// Any failure (network, timeout, 403, corrupt bytes, unknown codec) returns null
// so the card renders on its gradient fallback rather than failing entirely.
async function loadCardImage(imageUrl) {
  if (!imageUrl) return null;
  let origin = '';
  try { origin = new URL(imageUrl).origin; } catch { return null; }
  try {
    const r = await fetch(imageUrl, {
      // Browser-like headers mirror /api/img so the same tricky CDNs (e.g. NDTV)
      // that work for the live site also work here.
      headers: {
      ...
      redirect: 'follow',
      signal: AbortSignal.timeout(6000),   // generous: first render only, result is cached
    });
    if (!r.ok) return null;
    // NOTE: we deliberately do NOT gate on the Content-Type header. Supabase
    // sometimes serves uploads as application/octet-stream (empty file.type at
    // upload), which the browser renders fine but the old code rejected — that
    // was the "card came, but no image" bug. sharp inspects the actual bytes and
    // throws on a genuine non-image, which the catch below turns into a gradient.
    const buf = Buffer.from(await r.arrayBuffer());
    if (!buf.length || buf.length > 20_000_000) return null;  // sharp downsizes; cap is just memory safety

    const jpeg = await sharp(buf, { failOn: 'none', animated: false })
      .rotate()                                              // honour EXIF orientation
      .resize(IMG_W, IMG_H, { fit: 'cover', position: sharp.strategy.attention })
...
// Render to PNG, guaranteeing a card comes back. If satori/resvg trips on the
// image for any reason, retry once with no image (gradient) so a story is never
// un-shareable. Text always renders.
async function renderPngSafe(opts) {
  try {
    return await renderCard(opts);
  } catch (err) {
    if (opts.imageDataUri) {
      return await renderCard({ ...opts, imageDataUri: null });
    }
    throw err;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch + normalise the image directly. story.image is either a manually-
    // uploaded Supabase public URL or the publisher's own image URL — both are
    // fetched the same way and transcoded to a safe JPEG. null → gradient card.
    const wantImage    = !!(story.image && story.image.trim());
    const imageDataUri = wantImage ? await loadCardImage(story.image.trim()) : null;
    // "Healthy" = either the story genuinely has no image (gradient is correct
    // and permanent), or it has one and we loaded it. NOT healthy = an image was
    // expected but the fetch missed — that must NOT be cached for a week.
    const imageHealthy = !wantImage || !!imageDataUri;
    ...
    // Cache policy is the heart of the reliability fix. A good card is immutable,
    // so cache it hard. But a card whose image MISSED must expire fast, or a
    // single transient fetch failure freezes the gradient version for up to a
    // week — which is exactly the "card came without image" symptom.
    res.setHeader('Cache-Control', imageHealthy
      ? 'public, s-maxage=86400, stale-while-revalidate=604800'   // good card — cache hard
      : 'public, s-maxage=30, stale-while-revalidate=60');         // image missed — retry almost immediately
