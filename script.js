...
function setReaderMode(mode) {
  ...
  if (mode === 'web' && _rdrUrl) populateWeb(_rdrUrl);
  if (mode === 'reader' && _rdrUrl) fetchReader(_rdrUrl);   // de-dups via _rdrFetchedUrl
}

function populateWeb(url) {
  var openA = document.getElementById('rdr-web-open');
  if (openA) openA.href = url;
  var dom = document.getElementById('rdr-web-domain');
  if (dom) { try { dom.textContent = new URL(url).hostname.replace(/^www\./, ''); } catch (e) { dom.textContent = ''; } }
}
