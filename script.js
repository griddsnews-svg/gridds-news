...
function renderCardAd(){
  ...
  if(!ad || !ad.image){ slot.style.display='none'; return; }
...
/* ── READER OVERLAY — Web / Reader toggle ── */
var READER_API = '/api/reader';
var _rdrUrl = null, _rdrMode = 'reader', _rdrFetchedUrl = null, _rdrWebUrl = null;

function openReader(url, title, source, summary, imgSrc) {
  if (!url) return;
  _rdrUrl = url;
  var openA = document.getElementById('rdr-open'); if (openA) openA.href = url;
  var origA = document.getElementById('rdr-orig'); if (origA) origA.href = url;
  var fbA = document.getElementById('rdr-web-open'); if (fbA) fbA.href = url;
  setReaderMode('web');                    // open on the original page by default
  document.getElementById('rdr-overlay').classList.add('show');
}

function closeReader() {
  var ov = document.getElementById('rdr-overlay');
  if (ov) ov.classList.remove('show');
  var fr = document.getElementById('rdr-iframe');
  if (fr) { fr.src = 'about:blank'; _rdrWebUrl = null; }
}
...
function setReaderMode(mode) {
  _rdrMode = mode;
  document.querySelectorAll('#rdr-toggle .rdr-seg').forEach(function(b){
    b.classList.toggle('active', b.dataset.mode === mode);
  });
  var r = document.getElementById('rdr-reader');
  var w = document.getElementById('rdr-web');
  if (r) r.style.display = mode === 'reader' ? 'block' : 'none';
  if (w) w.style.display = mode === 'web' ? 'block' : 'none';
  if (mode === 'web' && _rdrUrl) loadWeb(_rdrUrl);
  if (mode === 'reader' && _rdrUrl) fetchReader(_rdrUrl);   // de-dups via _rdrFetchedUrl
}

function loadWeb(url) {
  var frame = document.getElementById('rdr-iframe');
  var fb = document.getElementById('rdr-web-fallback');
  if (!frame) return;
  if (fb) fb.classList.remove('show');
  if (_rdrWebUrl === url) return;          // already loaded
  _rdrWebUrl = url;
  var loaded = false;
  frame.onload = function(){ loaded = true; };
  frame.src = url;
  // Many publishers block framing (X-Frame-Options). If it never loads, offer the original.
  setTimeout(function(){ if (!loaded && fb) fb.classList.add('show'); }, 3500);
}

function fetchReader(url) {
  var load = document.getElementById('rdr-loading');
  var art  = document.getElementById('rdr-article');
  var err  = document.getElementById('rdr-error');
  if (_rdrFetchedUrl === url && art && art.style.display !== 'none') return;  // cached in DOM
  load.style.display = 'flex'; art.style.display = 'none'; err.style.display = 'none';
  fetch(READER_API + '?url=' + encodeURIComponent(url))
    .then(function(r){ return r.json(); })
    .then(function(d){
      load.style.display = 'none';
      if (!d || !d.ok || !d.content) throw new Error((d && d.error) || 'no content');
      document.getElementById('rdr-eyebrow').textContent = d.siteName || '';
      document.getElementById('rdr-title').textContent = d.title || '';
      var meta = [];
      if (d.byline) meta.push(d.byline);
      document.getElementById('rdr-meta').textContent = meta.join(' \u00b7 ');
      var hero = document.getElementById('rdr-hero');
      if (d.image) { hero.src = d.image; hero.style.display = 'block'; hero.onerror = function(){ hero.style.display = 'none'; }; }
      else hero.style.display = 'none';
      var body = document.getElementById('rdr-body');
      body.innerHTML = d.content;          // sanitised server-side
      body.querySelectorAll('a').forEach(function(a){ a.target = '_blank'; a.rel = 'noopener noreferrer'; });
      art.style.display = 'block';
      _rdrFetchedUrl = url;
      document.getElementById('rdr-reader').scrollTop = 0;
    })
    .catch(function(){
      load.style.display = 'none';
      err.style.display = 'block';
      err.innerHTML = 'We couldn\u2019t build a reader view for this article. <a href="' +
        url.replace(/"/g,'&quot;') + '" target="_blank" rel="noopener">Open the original \u2197</a>';
      _rdrFetchedUrl = null;
    });
}

(function wireReader(){
  var back = document.getElementById('rdr-back');
  if (back) back.addEventListener('click', closeReader);
  document.querySelectorAll('#rdr-toggle .rdr-seg').forEach(function(b){
    b.addEventListener('click', function(){ setReaderMode(b.dataset.mode); });
  });
})();
