...
window._griddsPrefetchCard = function(btn){
  ...
  if (btn._cardForId === btn._storyId && btn._cardFile) return;   // already have it
  btn._cardFile = null; btn._cardForId = btn._storyId;
  var id = btn._storyId;
  fetch('/api/share-card?id=' + encodeURIComponent(id))
    .then(function(r){ if (!r.ok) throw new Error('card'); return r.blob(); })
    .then(function(blob){
      if (btn._cardForId !== id) return;   // user moved on; ignore stale result
