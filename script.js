...
function flipBack(key){
  ...
  /* ─── LOCK NATIVE PULL-TO-REFRESH EVERYWHERE EXCEPT HOME ───
     The browser's OWN pull-to-refresh (separate from the custom gesture above)
     reloads the app on any downward overscroll — which is what was firing
     accidentally while swiping inside a story card or playing snake. We disable
     it by switching overscroll-behavior-y to 'contain' (kills the refresh +
     bounce, keeps normal scrolling) whenever any overlay/screen is open or while
     customising tiles, and restore 'auto' on the bare home grid so the intended
     home refresh still works. A MutationObserver watches the overlay class
     toggles, so no open/close call site needs to change.
     (overscroll-behavior is honoured on iOS 16+ and all current Android.) */
  (function(){
    var OVERLAY_SEL = '.story-expand.open, .story-expand.visible, .va-panel.open, ' +
      '.welcome-overlay.show, .profile-panel.show, .search-panel.show, ' +
      '#snake-overlay.show, #rdr-overlay.show';
    function locked(){
    ...
    // precise, no whole-subtree churn during the snake game.
    mo.observe(document.body, { attributes:true, attributeFilter:['class'] });
    ['.story-expand', '.va-panel', '.welcome-overlay', '.profile-panel',
     '.search-panel', '#snake-overlay', '#rdr-overlay'].forEach(function(sel){
...
window._griddsPrefetchCard = function(btn){
  ...
  // Store the in-flight promise so a fast tap can AWAIT it instead of falling
  // straight through to text-only. Because the fetch starts when the card
  // opens, by tap time it's usually already resolved (or milliseconds away),
  // keeping us inside iOS Safari's transient-activation window.
  btn._cardPromise = fetch('/api/share-card?id=' + encodeURIComponent(id) + '&theme=' + theme + '&v=2')
    .then(function(r){ if (!r.ok) throw new Error('card ' + r.status); return r.blob(); })
    .then(function(blob){
      var file = new File([blob], 'gridds-' + id + '.png', { type: 'image/png' });
      if (btn._cardForId === key) btn._cardFile = file;   // cache only if still relevant
      return file;                                        // always resolve for awaiters
    });
  btn._cardPromise.catch(function(){ /* leave _cardFile null → link fallback */ });
};
...
document.getElementById('se-btn-share').addEventListener('click', function(e) {
  ...
  /* Share the card image FILE if the platform supports it. Returns true if a
     share was initiated. */
  function shareFile(file) {
    if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
      navigator.share({ files: [file], text: caption, title: headline }).catch(function(){});
      return true;
    }
    return false;
  }
  /* Fallback: share the link (its og:image unfurls the same card), else copy. */
  function shareLink() {
    if (navigator.share) {
  ...
  /* 1) Card already prefetched → synchronous share. Happy path, fully iOS-safe. */
  if (shareFile(btn._cardFile)) return;

  /* 2) Prefetch still in flight → await it, then share. The fetch was started
        when the card opened, so it almost always resolves within the user-
        activation window. If it overruns, share() throws → link fallback (same
        as the old behaviour, so nothing regresses). */
  if (btn._cardPromise) {
    var old = btn.innerHTML; btn.innerHTML = '…';
    btn._cardPromise
      .then(function(file){ btn.innerHTML = old; if (!shareFile(file)) shareLink(); })
      .catch(function(){ btn.innerHTML = old; shareLink(); });
    return;
  }

  /* 3) Nothing prefetched at all → link fallback. */
  shareLink();
});
